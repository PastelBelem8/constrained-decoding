...
def parse_arguments() -> dict:
    ...
    #
    # Example use: python -m run_generate --config ./configs/generate-pythia70m.yml --model_name EleutherAI/pythia-70m
...
if __name__ == "__main__":
    ...
    prefix_col = sampling_configs.pop("prefix_col")
    for decoder_name, decoder_config in decoding_configs:
        ...
        start = time.time()
        results = generate(prefixes=data[prefix_col].values, model=model, tokenizer=tokenizer, **decoder_config)
        end = time.time()
        ...
        prefixes = data[prefix_col].values
